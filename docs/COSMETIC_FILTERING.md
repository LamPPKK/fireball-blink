...
2. Bind one policy/engine sequence to the owning Chromium Profile and discard
   every document plan on cross-document navigation or Profile teardown.
...
5. Apply the generic stylesheet only if the document token, Profile key,
   hostname and current site policy still match.
...
policy path with site-specific hiding, generic class matching, `$generichide`
and a site exemption. `tests/cosmetic_evaluator_test.cc` covers malformed JSON,
duplicate/unsorted output, unsafe CSS, cross-Profile plan reuse, disabled
generic matching and missing-engine failure.
